...
const jwksUrl = 'https://dev-xat6vsqh.us.auth0.com/.well-known/jwks.json'
...
async function verifyToken(authHeader: string): Promise<JwtPayload> {
  ...
  const secretObjects = await Axios.get(jwksUrl)

  const signingKeys = secretObjects.data.keys
    .filter(
      (key) =>
        key.use === 'sig' &&
        key.kty === 'RSA' &&
